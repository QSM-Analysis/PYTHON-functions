...
# .\dipole_kernel.m:76
    ...
    (Y,X,Z)=np.meshgrid(np.arange(-matrix_size/2,matrix_size/2), np.arange(-matrix_size/2,matrix_size/2), np.arange(-matrix_size/2,matrix_size/2))
# .\sphere_kernel.m:19
    X=dot(X,voxel_size)
# .\sphere_kernel.m:23
    Y=dot(Y,voxel_size)
# .\sphere_kernel.m:24
    Z=dot(Z,voxel_size)
# .\sphere_kernel.m:25
    a=abs(X) - dot(0.5,voxel_size)
    a[a<0]=0
    b=abs(Y) - dot(0.5,voxel_size)
    b[b<0]=0
    c=abs(Z) - dot(0.5,voxel_size)
    c[c<0]=0
    Sphere_out=(a ** 2 + b ** 2 + c ** 2) > (radius ** 2)
# .\sphere_kernel.m:26
    Sphere_in=((abs(X) + dot(0.5,voxel_size)) ** 2 + (abs(Y) + dot(0.5,voxel_size)) ** 2 + (abs(Z) + dot(0.5,voxel_size)) ** 2) <= radius ** 2
# .\sphere_kernel.m:30
...
# .\sphere_kernel.m:51
        zz=Z[i]
# .\sphere_kernel.m:52
        occupied=((xx + dot(X_v,voxel_size)) ** 2 + (yy + dot(Y_v,voxel_size)) ** 2 + (zz + dot(Z_v,voxel_size)) ** 2) <= radius ** 2
# .\sphere_kernel.m:54
        shell_val[i]=sum(occupied[:]) / np.size(X_v)
# .\sphere_kernel.m:57
    
    Sphere_mid=shell_val#原为Sphere_mid[shell==1]=shell_val
# .\sphere_kernel.m:60
