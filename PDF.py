...
# PDF.m:88
    b=p_temp[ravel(Mask) == 0]
# PDF.m:89
    # set erance level and maximum iteration allowed
    E_noise_level=np.real(np.fft.ifftn(multiply(D,np.fft.fftn(multiply(W_std,ones(np.shape(N_std)))))))
# PDF.m:92
    itermax=copy(n_CG)
# PDF.m:93
    A=lambda xx=None: (dipole_term(W_var,D,Mask,xx))
    
# PDF.m:94
    cg_tol=dot(tol,np.linalg.norm(E_noise_level[ravel(Mask) == 0])) / np.linalg.norm(ravel(b))
# PDF.m:95
    x,res,num_iter=cgsolve(A,b,cg_tol,itermax,0,nargout=3)
# PDF.m:97
    print('CG stops at: res %f, iter %d\n',res,num_iter)
    xx=np.zeros(np.size(D))
# PDF.m:100
    xx[ravel(Mask) == 0]=x(arange(1,-1))
# PDF.m:101
    xx[ravel(Mask) > 0]=0
# PDF.m:102
